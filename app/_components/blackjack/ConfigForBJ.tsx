...
export default function ConfigForBJ({
  ...
  const handleDeal = () => {
    ...
    onBet(betAmount);
    setCooldown(true);
    setTimeout(() => setCooldown(false), 700); // brief debounce so users don't spam
  };
  ...
  return (
    <div className="flex flex-col gap-6 p-4 text-white max-w-md mx-auto rounded-2xl bg-slate-900/60 border border-slate-800">
      ...
      >
        {handInProgress ? "Hand in progress..." : cooldown ? "Please wait..." : betAmount > (balance ?? 0) ? "Insufficient Balance" : "Deal"}
      </button>

      <div className="text-xs text-slate-400">
        Blackjack pays 3:2 · Dealer stands on soft 17 · Double Down available on first move if you have funds.
      </div>
    </div>
  );
}
